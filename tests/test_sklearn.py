...
class TestKabenaWrapper:
    ...
    def test_fit_predict_classification(self, clf_data):
        ...
        model = KabenaWrapper(LogisticRegression(max_iter=1),
                              K=0.20, N=0.0, epochs=50, task="classification")
        model.fit(X_tr, y_tr)
    ...
    def test_fit_predict_regression(self, reg_data):
        ...
        model = KabenaWrapper(Ridge(alpha=1.0),
                              K=0.30, N=0.0, epochs=50, task="regression")
        model.fit(X_tr, y_tr)
        preds = model.predict(X_te)
        assert len(preds) == len(y_te)
    ...
    def test_stats_populated(self, clf_data):
        X_tr, X_te, y_tr, y_te = clf_data
        model = KabenaWrapper(LogisticRegression(max_iter=1),
                              K=0.20, N=0.0, epochs=30, task="classification")
        model.fit(X_tr, y_tr)
    ...
    def test_N1_equivalent_to_standard(self, clf_data):
        ...
        X_tr, X_te, y_tr, y_te = clf_data
        model = KabenaWrapper(LogisticRegression(max_iter=1),
                              K=0.50, N=1.0, epochs=50, task="classification")
        model.fit(X_tr, y_tr)
        # Gain doit être ~0 (toutes les mineures conservées)
        assert model.stats_["mean_gain_pct"] < 2.0, \
               "N=1 → aucune exclusion → gain ≈ 0%"

    def test_K_auto(self, clf_data):
        """K='auto' doit fonctionner sans erreur."""
        X_tr, X_te, y_tr, y_te = clf_data
        model = KabenaWrapper(LogisticRegression(max_iter=1),
                              K="auto", N=0.0, epochs=30, task="classification")
        model.fit(X_tr, y_tr)
        assert model.K_ > 0
